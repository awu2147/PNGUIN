...
namespace PNGUIN
{
    ...
    {
        public uint ChunkLength { get { return (uint)ChunkData.Length; } }
        ...
        public uint ChunkCRC { get { return CalculateCRC(); } }
        ...
        public byte[] Chunk
        {
            get
            {
                ...
                byte[] lengthBits = PNGUtils.GetBytes(ChunkLength);
                byte[] typeBits = Encoding.UTF8.GetBytes(ChunkType);
        ...
        public uint CalculateCRC()
        {
            ...
            CRC.UpdateCRC(ref crc, Encoding.UTF8.GetBytes(ChunkType), false);
            CRC.UpdateCRC(ref crc, ChunkData, true);
            return crc;
        }
    ...
    public class IHDRChunk : PNGChunk
    {
        ...
        private static readonly byte[] AllowedBitDepths = { 1, 2, 4, 8, 16 };
        ...
        public override byte[] ChunkData
        {
            ...
            set
            {
                ...
                var bitDepth = PNGUtils.ParseByte(value, ref offset);
                BitDepth = AllowedBitDepths.Contains(bitDepth) ? bitDepth : throw new ApplicationException("Bit depth is not supported");
                var colorType = PNGUtils.ParseByte(value, ref offset);
                ColorType = AllowedColorTypes.Contains(colorType) ? colorType : throw new ApplicationException("Colour type is not supported");
